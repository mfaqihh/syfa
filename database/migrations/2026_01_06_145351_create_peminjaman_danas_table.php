...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('peminjaman_dana', function (Blueprint $table) {
            ...
            $table->integer('tenor_peminjaman')->nullable();
            $table->decimal('presentase_bagi_hasil', 10, 2);
            $table->decimal('pps');
            $table->decimal('s_finance');
            $table->decimal('pembayaran_perbulan', 15, 2);
            $table->decimal('total_jumlah_peminjaman', 15, 2);
            $table->decimal('total_nominal_dialihkan', 15, 2);
            $table->decimal('total_peminjaman_disetujui', 15, 2);
            $table->decimal('total_dibayarkan', 15, 2)->default(0);
            $table->string('status_peminjaman')->default('draft');
            $table->boolean('is_active')->default(true);
            $table->string('bukti_transfer')->nullable();
            $table->string('nomor_kontrak')->nullable();
            $table->date('tanggal_dicairkan');
            $table->timestamps();
        });
    }
